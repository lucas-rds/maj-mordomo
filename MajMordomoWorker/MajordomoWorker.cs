    ...
    public class MajordomoWorker : IDisposable
    {
        ...
        public void SendToBroker(string command, string option, ZMessage msg)
        {
            using (msg = msg != null ? msg.Duplicate() : new ZMessage())
            {
                ...
                if (Verbose)
                    msg.LogEachFrame("I: sending '{0:X}|{0}' to broker", command.ToMdCmd());
        ...
        public void ConnectToBroker()
        {
            ...
            if (Verbose)
                "I: connecting to broker at {0}...".Log(Broker);
        ...
        public MajordomoWorker(string broker, string service, bool verbose = true)
        {
            if (broker == null)
            ...
            while (true)
            {
                ...
                if (Worker.PollIn(p, out msg, out error, Heartbeat))
                {
                    using (msg)
                    {
                        // If we got a reply
                        if (Verbose)
                            msg.LogEachFrame("I: received message from broker:");
                        ...
                        using (ZFrame command = msg.Pop())
                        {
                            ...
                            else if (command.StrHexEq(MdpCommon.MdpwCmd.DISCONNECT))
                            {
                                ...
                                "E: invalid input message: '{0}'".Log(command.ToString());
                        }
                    }
                }
                ...
                {
                    if (Verbose)
                        "W: disconnected from broker - retrying...".Log();
